...
namespace AbstractRepairWorkView
{
    public class MailCustomer
    {
        ...
        private static StreamReader reader; private static StreamWriter writer;
        ...
        /// <param name="message"></param>/// <param name="errorMessage"></param>
        /// <returns></returns>
       ...
       string message, string errorMessage)
        ...
        private static void GetMessages(int number)
        {
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        APICustomer.PostRequest<InfoMessageBindingModel,
                       bool>("api/InfoMessage/AddElement", new InfoMessageBindingModel
                       {
                           MessageId = messageId,
        ...
        private static string GetBody(string response, string coding)
        {
            ...
            response = reader.ReadLine();
            StringBuilder bodyMessage = new StringBuilder(); bool needEncoding = false;
